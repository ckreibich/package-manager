...
def git_remote_urls(repo):
    """Returns a map of remote name -> URL string entries for configured remotes.

    You'd normally use repo.remotes[n].urls for this, but with git versions (<2.7,
    e.g. on CentOS 7 at the time of writing) this triggers a "git remote show"
    (without -n) that will query the remotes, which can fail test cases. This
    uses the config subsystem to query URLs directly -- one of the fallback
    mechanisms in GitPython's Remote.urls() implementation.
    """
