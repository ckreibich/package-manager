...
class Config(configparser.ConfigParser):
    ...
    def save(self, configfile: str | None = None) -> bool:
        ...
        if not configfile:
            print_error("error: unable to locate a file to save the configuration to")
            return False
    ...
    def set(self, section: str, option: str, val: str) -> None:
        """Like ConfigParser's set(), but creating sections on the fly."""
        if not self.has_section(section):
            self.add_section(section)
        super().set(section, option, val)

    def set_interactively(section: str, option: str, val: str, prompt=True) -> None:
    ...
    def zeek_dist(self) -> str:
        ...
        return self.get("paths", "zeek_dist", fallback=None)
    ...
    def find_configfile(self, args: argparse.Namespace) -> str | None:
        ...
        Returns the path to the first config file successfully located, and None
        ...
        if args.user:
            configfile: str | None = os.path.join(self.home_config_dir(), "config")
